...
function installNpmPackagesAndBuildProdClient() {
  ...
  console.log(
    " (1/2) - Installing dependencies (script path: " + scriptPath + ")"
  );
  child_process.execSync(
    `cd ${scriptPath} && npm run install-all && npm run build-prod`
  );
  console.log(" (1.5/2) - Done installing dependencies");
}

function startServer() {
  const startServerFileName = PRODUCTION
    ? "startInProduction.js"
    : "startInDevelopment.js";
  ...
  console.info(
    " (2/2) - Starting server using " + (USING_PM2 ? "pm2" : "node")
  );
  if (USING_PM2) {
